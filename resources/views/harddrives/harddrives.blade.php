        ...
        <div class="row align-items-end">
            <div class="col-lg-8">
                <div class="page-header-title">
                    <div class="d-inline">
                        <h4>Hard Drives</h4>
                        <span>All Hard Drives at the Harare Depot</span>
                    </div>
                </div>
            </div>
            <div class="col-lg-4">
                <div class="page-header-breadcrumb">
                    <ul class="breadcrumb-title">
                        ...
                        <li class="breadcrumb-item"><a href="{{url('/harddrives')}}">Hard Drives</a>
                        </li>
                    </ul>
                </div>
            </div>
    ...
    <div class="page-body">
        <div class="card">
            <div class="card-header">
                <h5>Hard Drives</h5>
                <span>Asset Hard Drives</span>
                <div class="card-header-right">
                    <ul class="list-unstyled card-option">
                        <a href="{{ url('harddrives/create') }}" class="btn btn-round btn-light float-right">
                ...
                <div class="dt-responsive table-responsive">
                    <table id="vince-tables" class="table table-striped table-bordered nowrap" >
                        ...
                        @foreach($harddrives as $harddrive)
                            <tr>
                                <td>{{$harddrive->assettag}}</td>
                                <td>{{$harddrive->brand}}</td>
                                <td>{{$harddrive->model}}</td>
                                <td>{{$harddrive->capacity}}</td>
                                <td style="white-space: nowrap;">
                                    {!! Form::open(array('url' => 'harddrives/' . $harddrive->id, 'class' => 'btn btn-mini')) !!}
                                    {!! Form::hidden('_method', 'DELETE') !!}
                                    {!! Form::button('<i class="feather icon-trash-2"></i>', array('class' => 'btn btn-danger btn-mini','type' => 'button','data-toggle' => 'modal', 'data-target' => '#confirmDelete', 'data-title' => 'Delete Hard Drive', 'data-message' => 'Are you sure you want to delete this hard drive ?')) !!}
                                    {!! Form::close() !!}

                                    <a class="btn btn-success btn-mini" href="{{ URL::to('harddrives/' . $harddrive->id) }}" >
                                        <i class="feather icon-eye"></i>
                                    </a>

                                    <a class="btn btn-primary btn-mini" href="{{ URL::to('harddrives/' . $harddrive->id . '/edit') }}" >
