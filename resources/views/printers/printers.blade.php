        ...
        <div class="row align-items-end">
            <div class="col-lg-8">
                <div class="page-header-title">
                    <div class="d-inline">
                        <h4>Printers</h4>
                        <span>Printers summary info list</span>
                    </div>
                </div>
            </div>
            <div class="col-lg-4">
                <div class="page-header-breadcrumb">
                    <ul class="breadcrumb-title">
                        ...
                        <li class="breadcrumb-item"><a href="{{url('/printers')}}">Printers</a>
                        </li>
                    </ul>
                </div>
            </div>
    ...
    <div class="page-body">
        <div class="card">
            <div class="card-header">
                <h5>Printers</h5>
                <span>Asset Printers</span>
                <div class="card-header-right">
                    <ul class="list-unstyled card-option">
                        <a href="{{ url('printers/create') }}" class="btn btn-round btn-light float-right">
                ...
                <div class="dt-responsive table-responsive">
                    <table id="vince-tables" class="table table-striped table-bordered nowrap" >
                        ...
                        @foreach($printers as $printer)
                            <tr>
                                <td>{{$printer->assettag}}</td>
                                <td>{{$printer->type}}</td>
                                <td>{{$printer->brand}}</td>
                                <td>{{$printer->model}}</td>
                                <td style="white-space: nowrap;">
                                    {!! Form::open(array('url' => 'printers/' . $printer->id, 'class' => 'btn btn-mini')) !!}
                                    {!! Form::hidden('_method', 'DELETE') !!}
                                    {!! Form::button('<i class="feather icon-trash-2"></i>', array('class' => 'btn btn-danger btn-mini','type' => 'button','data-toggle' => 'modal', 'data-target' => '#confirmDelete', 'data-title' => 'Delete Printer', 'data-message' => 'Are you sure you want to delete this printer ?')) !!}
                                    {!! Form::close() !!}

                                    <a class="btn btn-success btn-mini" href="{{ URL::to('printers/' . $printer->id) }}" >
                                        <i class="feather icon-eye"></i>
                                    </a>

                                    <a class="btn btn-primary btn-mini" href="{{ URL::to('printers/' . $printer->id . '/edit') }}" >
