...
class ProfilesController extends Controller
{
    ...
    public function show($username)
    {
        ...
        $assets = Asset::where('username', $user->paynumber)->get();
        $logs = Maintenance::where('username', $user->name)->get();
    ...
    public function getChangePwdForm(){
        $user = Auth::user();
        return view('usersmanagement.change-my-password', compact('user'));
    }

    public function updateMyPassword(UpdateUserPasswordRequest $request, $id)
    {
        $currentUser = \Auth::user();
        $user = User::findOrFail($id);
        $ipAddress = new CaptureIpTrait();
